...
def demoSingleKeyJava = { Dynamo dynamo ->
    final Map customerKey = [customerId: UUID.randomUUID().toString()]
    assert !dynamo.listTables()
    ...
    final Map samAttrs = [name: 'Sam', address: '456 My Way', age: 40]
    final Map sam = customerKey + samAttrs
    table.upsert(new Consumer<Ops.Upsert>() {
	void accept(Ops.Upsert up) {
	    up.key(customerKey)
	    up.attributes(samAttrs)
	}
    })
    ...
    //get the same information via the query interface
    assert sam == table.query(new Consumer<Ops.Query>() {
	void accept(Ops.Query q) {
	    q.keyCondition('{} = {}', q.alias('customerId'), customerKey.customerId)
	}
    })[0]
	...
	void accept(Ops.Upsert up) {
	    up.key(upsertKey)
	    up.expression("set {} = {} remove {}", up.alias('name'), lenny.name, up.alias('age'))
	}
    })
    assert lenny == table.get(upsertKey)
...
def demoMultipleKeysJava = { Dynamo dynamo ->
    assert !dynamo.listTables().find { name -> name == "Invoices" }
    ...
    //very large. Iterable will automatically paginate results. If you want it
    //all at once, cast the Iterable to a List.
    def iterable = invoices.query(new Consumer<Ops.Query>() {
	void accept(Ops.Query q) {
	    q.projection('{}, {}, {}', q.alias('sk'), q.alias('price'), q.alias('quantity'))
	    q.keyCondition('{} = {} AND begins_with({}, {})', q.alias('pk'), invoiceId, q.alias('sk'), '#li')
	}
    })
    ...
    //This is requires the same work on the server (since all items are scanned),
    //but reduces the I/O since only some items are brought back
    def results = invoices.scan(new Consumer<Ops.Scan>() {
	void accept(Ops.Scan s) {
	    s.projection('{},{}', s.alias('quantity'), s.alias('price'))
	    s.filter('begins_with({}, {}) AND {} > {}', s.alias('sk'), '#li', s.alias('price'), 20)
	}
    }) as List
...
def demoTransactionsJava = { Dynamo dynamo ->
    final faddr = faker.address()
    final fname = faker.name()
	...
	void accept(Ops.WriteTransaction tran) {
	    tran.put(person, "People")
	    for(Map<String,Object> addr : addresses) {
		String id = addr.id
		def attrs = addr - [id: id]
		tran.upsert(new Consumer<Ops.Upsert>() {
    ...
    //Dynamo multi-table/multi-row read transaction
    List<Map<String,Object>> all = dynamo.readTransaction(new Consumer<Ops.ReadTransaction>() {
	void accept(Ops.ReadTransaction tran) {
	    tran.get(Map.of('id', personId), 'People')
	    for(Map<String,Object> addr : addresses) {
		tran.get(Map.of('id', addr.id), 'Addresses')
	    }
	}
    })
    ...
    dynamo.writeTransaction(new Consumer<Ops.WriteTransaction>() {
	void accept(Ops.WriteTransaction tran) {
	    tran.check(new Consumer<Ops.Check>() {
		void accept(Ops.Check check) {
		    check.table("People")
		    check.key(Map.of('id', personId))
		    check.condition("{} = {}", check.alias('ssn'), person.ssn)
		}
	    })

	    tran.delete(Map.of('id', addresses[0].id), 'Addresses')
	    tran.delete(Map.of('id', addresses[1].id), 'Addresses')
	}
    })
    
    List<Map<String,Object>> reduced = dynamo.readTransaction(new Consumer<Ops.ReadTransaction>() {
	void accept(Ops.ReadTransaction tran) {
	    for(Map<String,Object> addr : addresses) {
		tran.get(Map.of('id', addr.id), 'Addresses')
	    }
...
try(def env = builder.inMemory()) {
    ...
    demoMultipleKeysJava dynamo
    demoTransactions dynamo
    demoTransactionsJava dynamo
    demoAt dynamo
